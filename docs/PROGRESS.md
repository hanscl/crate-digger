...
## LAB-47 — Tighten overgeneralized genre slots

- **Status:** in review
- **Branch:** `lab-47-c-tighten-overgeneralized-genre-slots-substring-matching`
- **PR:** _pending_
- **Problem:** the genre half of every embedding matched slots by contiguous
  token-subsequence, so the bare `rock` slot (shared by metal/hard-rock) fired
  for "pop rock" and "indie rock". Pop/indie tracks borrowing the rock idiom
  gained a spurious `rock` dimension, giving them cross-genre affinity with the
  rock-vs-metal lane (e.g. a pop-rock act overlapping the metal bucket via the
  shared slot). Acceptance: "pop rock" must NOT light bare `rock` while
  legitimate "rock"/"hard rock"/"classic rock"/"punk rock" still do; multi-word
  keywords already in the table ("alt rock", "post rock", "drum and bass", …)
  must still match (word/token-aware, not naive substring).
- **Matcher change (`src/lib/embedding.ts`):** added an optional
  `blockedByTokens` field to each genre slot def and a shared `tagMatchedKeyword`
  helper. The bare `rock` slot is `blockedByTokens: ["pop", "indie"]`: a slot's
  keyword match is suppressed **only for the single tag that ALSO carries a
  cross-family qualifier token**. The block is PER-TAG, not per-track — a track
  tagged both "pop rock" and bare "rock" still lights `rock` from the bare tag;
  only the qualified tag is routed to its own lane. The qualifier still lights
  its own slot (`pop`/`indie`), so the genre signal is rerouted, not lost.
  `tagMatchedKeyword` is shared by `genresToHotVector` (multi-hot) and
  `derivePrimaryGenre` (primary label) so the two can never disagree on what a
  tag matches. Verified: "pop rock"→pop (no rock), "indie rock"→indie (no rock),
  "indie pop rock"→indie+pop (no rock); "rock"/"hard rock"/"classic rock"/
  "punk rock"/"rock & roll" still light rock; "alt rock"→alternative, "post
  rock"→post-rock+rock all preserved.
- **Reviewed the prior agent's change:** the matcher approach (token-aware
  per-tag block) is sound and kept. But the prior agent's TEST edits were
  **broken** and never run: they switched the cross-lane fixtures to "punk rock"
  and asserted `primaryGenre === "punk"`. "punk rock" actually derives a `rock`
  PRIMARY (its "punk" and "rock" keywords tie at length 4 and `rock` is iterated
  first), so those assertions would fail AND "punk rock" shares the bucket's
  primary lane — defeating the cross-lane premise. Corrected all cross-lane
  fixtures to **"blues rock"** (primary `blues` — "blues" is the longest matched
  keyword — yet still lights the bare `rock` slot, so it genuinely bridges lanes
  the exact gate would split).
- **Blast radius (scripted over the LAB-36 cohort, 136 tracks):** exactly **1**
  track changes its multi-hot vector and **0** change primary genre. The per-tag
  block is why it's so small — nearly every "pop rock"/"indie rock" track in the
  cohort also carries a bare "rock" tag that still lights `rock`. The one change
  is **"Malcolm Todd — Earrings"** (alt-rnb/indie/funk/soul), whose ONLY rock
  signal was the "indie rock" tag → it loses the spurious `rock` slot (keeps
  indie/alternative/pop/rnb/soul/funk). This is precisely the bug LAB-47 fixes.
- **Fixture/assertion updates (with justification):**
  - `tests/embedding.test.ts` — kept the prior agent's "punk rock" multi-hot
    cases (correct: "punk rock" DOES light both `rock` and `punk`; only its
    _primary_ tie-breaks to rock, which these tests don't assert) and the new
    LAB-47 cases (pop/indie no-bleed, genuine-rock-family still lights,
    per-tag block).
  - `tests/bucketing/assign.test.ts` + `tests/bucketing/genre-scope.test.ts` —
    cross-lane fixture switched "punk rock"→"blues rock" so the asserted
    primary (`blues`) is truthful and the lane is genuinely crossed.
  - `tests/surfacing/pipeline.test.ts` (2 tests) + `tests/evals/counterfactual.test.ts`
    (1 test) — these relied on "indie rock" sharing the bare `rock` slot to let
    a cross-lane candidate win a refill slot under slot-overlap. After LAB-47
    "indie rock" no longer shares it, so the premise broke; switched the
    candidate to "blues rock" (still shares `rock`, blues primary). The
    legacy/exact-gate halves still exclude it (blues ≠ rock).
  - `tests/evals/lab36-reassignment-replay.test.ts` — **CONTROL** (exact gate +
    plain cosine) bucket count **32 → 33**. The +1 is "Earrings" above: dropping
    its spurious `rock` slot lowers its plain cosine to the rock-heavy
    `alternative` lane it previously joined below the 0.7 spawn threshold, so it
    spawns its own `alternative` bucket. The old 32-count was a property of the
    BUGGY embeddings; tighter genre separation is the point. The qualitative
    CONTROL invariants (MTW metal-locked WITH its banger; SMG/Extrabreit apart;
    Shins/BoH apart) and ALL the NEW-config sanity guards (bucket count in
    [8,40], max-share < 0.7, null-lane purity, null-audio spread, identical-
    audio disjoint-slot pair spawns apart) are untouched and still pass. No
    byte-exact legacy-replay invariant (the `weightedCosine(a,b,1) === cosine`
    IEEE-754 identity) and no Constraint #2/#4/#5 guard was weakened.
- **Embedding staleness (decision: option (b) — re-derive on next enrichment):**
  stored `track.embedding` rows are now stale. There is no dedicated re-embed
  script and none is added — the enrichment steps (`lastfm-tags.ts`,
  `musicbrainz.ts`, `discogs.ts`) already call `buildEmbedding` and rewrite
  `track.embedding` + `primary_genre` on every enrichment pass (the LAB-22/23
  pattern), so rows re-derive automatically on the next enrichment run. A full
  immediate rebuild + re-bucketing of LIVE members is the operator's call and a
  **follow-up**, mirroring the LAB-36 "re-bucketing live members is a follow-up"
  note. **No `model_version` bump** — this is a feature/embedding change, not a
  ranker-config change (Constraint #3 untouched).
- **Gate:** `pnpm check` clean (0/0), `pnpm typecheck` clean, `pnpm test`
  **320 passed (33 files)** — includes the LAB-36 eval/replay tests under
  `tests/evals/` over `tests/fixtures/lab36-cohort.json`.
