...
## LAB-76 — Neutral settles a track + recs panel readability

- **Status:** in review
- **Branch:** `lab-76-neutral-settles-a-track-queue-n-action-recs-panel-highlight`
- **PR:** _pending_
- **Problem:** (1) the queue had no way to dismiss a track you've seen and are
  indifferent to — the only "make it go away" action was defer, which
  re-surfaces, so genuinely uninteresting tracks kept reappearing. (2) the
  recommendations panel rendered raw bucket ids + a JSON blob, unreadable at a
  glance and with no link to the bucket you were inspecting.
- **Scope landed — Part 1 (neutral settles a track):**
  - `neutral` was ALREADY a valid `RatingDecision` (schema enum) and already
    zero-signal everywhere it mattered — feedback (`ingest-rating.ts`: no bucket
    commit / dislike counter), retrain, and the keep-rate / agreement eval
    metrics all exclude defer+neutral. LAB-76's only behavioral change is at the
    **surfacing-entry eligibility gate**: `loadIneligibleTrackIds`
    (`surfacing/pipeline.ts`) now treats `neutral` as decided alongside
    `keep`/`dislike` (`inArray(decision, ["keep","dislike","neutral"])`), so a
    neutral-rated track never re-surfaces. `defer` stays eligible (defer =
    "later", re-surfaces on subsequent runs).
  - Queue UI (`queue.tsx`): new 4th action `N` = neutral. J/K/L untouched; the
    keydown handler, the on-screen button (ghost style, "seen it, indifferent —
    never re-surfaces, no taste signal" title), the `onRate` decision union, and
    the header hint all gained `neutral`. The rate router already accepted the
    enum value — no server change needed for Part 1.
  - **No `model_version` bump** — neutral carries zero taste signal, so it is
    not a ranker/config change (Constraint #3 untouched). Soft-penalty
    Constraint #4 untouched: the gate is decision dedupe, not a taste filter.
  - Docs: Constraint #5 (`CLAUDE.md`) + the surfacing section in `docs/PLAN.md`
    amended "keep/dislike-decided" → "keep/dislike/neutral-decided (amended
    LAB-60/LAB-76)" with the "neutral = seen-it-indifferent, zero taste signal"
    gloss. Both files now consistent.
- **Scope landed — Part 2 (recs panel: names + selection-awareness + readability):**
  - `buckets.recommendations` (`server/routers/buckets.ts`) now joins bucket
    NAMES (+ color) into the payload: one `select … inArray(bucket.id, …)` over
    the distinct referenced ids, mapped onto each rec as a `buckets` array of
    `{ id, name, color }`. Name is **nullable per id** — a concurrent
    merge/removeMember can
    prune a referenced bucket before the next recompute. Panel stays GLOBAL
    (lists every pending merge/split); selection only reorders/highlights.
  - Pure helpers extracted to `src/web/screens/buckets-recs.ts` (node-testable,
    mirrors `queue-urls.ts`): `recInvolvesBucket`, `orderRecommendations`
    (stable partition — recs touching the selected bucket hoist to top,
    server order preserved within groups, never mutates), `bucketRefLabel`
    (name or `#id` fallback), and `formatRecommendationReason` (one deterministic
    sentence per shape, verified against the real `reason` JSON from
    `bucketing/recommendations.ts`: merge = `{similarity, threshold}` →
    "Centroids are 0.95 similar (≥ 0.92 merge bar) — merge A + B."; split =
    `{memberCount, dislikeCount, dislikeRate, threshold}` → "63% dislike rate
    (5/8 disliked) exceeds the 50% split bar — split X."). Unexpected shapes
    degrade to a generic line; raw JSON stays available.
  - `buckets.tsx`: renders clickable bucket-name buttons (click selects that
    bucket — `setInternalSelected` + `setLocation`), accent border + soft bg +
    sort-to-top for recs involving the selected bucket, the reason SENTENCE, and
    the raw `reason` JSON behind a collapsed `<details>` toggle.
- **Tests (330 green; +13 new):** surfacing — neutral excluded at entry (absent
  from `candidate_pool`, no bucket/member side effect), reusing the LAB-60
  disliked-track harness; the existing defer-re-surfaces test still passes
  unchanged. Pure helpers — `recInvolvesBucket`, `orderRecommendations`
  (hoist + stable + no-mutation + null selection), `bucketRefLabel` (name +
  `#id` fallback), `formatRecommendationReason` (merge/split exact strings,
  `#id` fallback, generic-degrade). Queue `rate` already had `neutral` coverage
  via the enum; the surfacing test is the load-bearing new behavior.
- **Decisions locked:**
  - neutral is a **surfacing-gate** change only, NOT a taste signal — it reuses
    the LAB-60 dedupe gate. defer ≠ neutral: defer re-surfaces, neutral settles.
  - The recs panel stays GLOBAL; selection-awareness is presentation
    (highlight + sort), it does not filter the list.
- **Notes for future phases:**
  - No migration; no version bump. Reason-sentence phrasing is pinned to the
    current `recommendations.ts` reason shapes — if those keys change, update
    `formatRecommendationReason` (it degrades gracefully, but the nice sentence
    would silently fall back to the generic line).
